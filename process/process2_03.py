...
def write_colmap_sparse(cameras_dict, pts3d, confidence, image_paths, output_dir):
    ...
    with open(cameras_file, 'w') as f:
        ...
        for cam_id, cam_params in cameras_dict.items():
            # COLMAP PINHOLE model: fx, fy, cx, cy
            f.write(f"{cam_id} PINHOLE {cam_params['width']} {cam_params['height']} "
                   f"{cam_params['fx']} {cam_params['fy']} {cam_params['cx']} {cam_params['cy']}\n")
    ...
    with open(images_file, 'w') as f:
        ...
        for cam_id, cam_params in cameras_dict.items():
            ...
            # Image name
            img_name = os.path.basename(image_paths[cam_id])
            
            # Write image line
            f.write(f"{cam_id} {quat[0]} {quat[1]} {quat[2]} {quat[3]} "
                   f"{t[0]} {t[1]} {t[2]} {cam_id} {img_name}\n")
            
            # Write empty points2D line (no 2D-3D correspondences for now)
            f.write("\n")
