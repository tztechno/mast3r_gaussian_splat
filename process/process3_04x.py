...
def extract_scene_data(scene, min_conf_thr: float, verbose: bool) -> Tuple[Dict, Dict, Dict]:
    ...
    all_pts3d = scene.get_pts3d()  # 修正: 全3D点を一度に取得
    ...
    for idx in range(num_images):
        ...
        # カメラポーズ推定
        pts3d = all_pts3d[idx]  # 修正: リストから取得
...
def estimate_camera_pose(pts3d: np.ndarray, confidence: np.ndarray, min_conf_thr: float) -> np.ndarray:
    ...
    Returns:
        4x4変換行列
    """
    # テンソルをNumPy配列に変換（勾配を切り離す）
    if hasattr(pts3d, 'cpu'):  # PyTorchテンソルの場合
        pts3d = pts3d.detach().cpu().numpy()
    if hasattr(confidence, 'cpu'):  # PyTorchテンソルの場合
        confidence = confidence.detach().cpu().numpy()
    
    # 3D配列を2D配列に変換してからマスクを適用
    h, w = pts3d.shape[:2]
    pts_flat = pts3d.reshape(-1, 3)  # [H*W, 3]に変換
    conf_flat = confidence.reshape(-1)  # [H*W]に変換
