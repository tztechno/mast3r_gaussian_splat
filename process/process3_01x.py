...
"""
MASt3R to COLMAP Conversion - Modular Functions
================================================
クラスなしの個別関数として実装
各関数を独立して使える
"""
...
def extract_scene_data(scene, min_conf_thr: float, verbose: bool) -> Tuple[Dict, Dict, Dict]:
    ...
    Returns:
        (cameras, images_data, points3D) のタプル
    """
    cameras = {}
    images_data = {}
...
def extract_3d_points(scene, min_conf_thr: float, verbose: bool) -> Dict:
    ...
    Returns:
        3D点の辞書 {point_id: {id, xyz, rgb, error, image_ids, point2D_idxs}}
    """
    points3D = {}
    point_id = 1
...
def save_image_data(
    ...
    Args:
        scene: MASt3Rシーン
        images_dir: 画像保存ディレクトリ
        depth_dir: デプスマップ保存ディレクトリ
        normal_dir: ノーマルマップ保存ディレクトリ
        mask_dir: マスク保存ディレクトリ（Noneの場合は保存しない）
        min_conf_thr: 最小信頼度閾値
        verbose: 詳細出力
    """
    num_images = len(scene.imgs)
