...
def export_colmap_binary_with_colors(cameras_dict, pts3d, confidence, colors, image_size, output_dir):
    ...
    # Collect unique camera parameters
    unique_cameras = {}
    img_to_cam_id = {}
    
    for img_id, (img_name, cam_data) in enumerate(cameras_dict.items(), 1):
        cam_key = (
            round(cam_data['focal'], 2),
            round(cam_data['pp'][0], 2),
            round(cam_data['pp'][1], 2)
        )
        
        if cam_key not in unique_cameras:
            # Ensure camera IDs are assigned in the order they are added
            unique_cameras[cam_key] = {
                'focal': cam_data['focal'],
                'pp': cam_data['pp']
            }
        
        # Use the insertion index as the camera ID
        cam_id = list(unique_cameras.keys()).index(cam_key) + 1
        img_to_cam_id[img_name] = cam_id
    
    # 1. Write cameras.bin
    cameras_bin_path = os.path.join(output_dir, "cameras.bin")
    with open(cameras_bin_path, 'wb') as f:
        write_binary(f, len(unique_cameras), 'Q')
        
        # Use key list to maintain order
        for cam_id, cam_key in enumerate(unique_cameras.keys(), 1):
            cam_info = unique_cameras[cam_key]
            
            # Debug output
            if cam_id <= 3:
                print(f"  Writing camera {cam_id}: focal={cam_info['focal']:.2f}")
            
            write_binary(f, cam_id, 'I')  # Write sequential IDs
            write_binary(f, 1, 'i')  # SIMPLE_PINHOLE model
            write_binary(f, image_size[0], 'Q')
            write_binary(f, image_size[1], 'Q')
            write_binary(f, cam_info['focal'], 'd')
            write_binary(f, cam_info['pp'][0], 'd')
            write_binary(f, cam_info['pp'][1], 'd')
    
    print(f"✓ cameras.bin: {len(unique_cameras)} unique cameras")
    ...
    with open(images_bin_path, 'wb') as f:
        ...
        for img_id, (img_name, cam_data) in enumerate(cameras_dict.items(), 1):
            ...
            cam_id = img_to_cam_id[img_name]
            write_binary(f, cam_id, 'I')
            
            write_string(f, img_name)
            write_binary(f, 0, 'Q') # Number of points2D (set to 0 for export)
    ...
    with open(points3d_bin_path, 'wb') as f:
        ...
        for point_id, (pt, conf, color) in enumerate(zip(pts3d, confidence, colors), 1):
            ...
            # Convert confidence to error
            error = 1.0 / max(conf, 0.01)
            write_binary(f, error, 'd')
            
            write_binary(f, 0, 'Q') # Track length (set to 0 for export)
