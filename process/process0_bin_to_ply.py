def generate_pointcloud_from_colmap_bins_robust(colmap_sparse_dir, output_ply_path, colorize_by_height=False):
    ...
    try:
        with open(points3d_bin, 'rb') as f:
            # ❌ Removed buggy code:
            # num_points_bytes = f.read(8)  # <- This was incorrect!
            # num_points = struct.unpack('Q', num_points_bytes)[0]
            
            # ✅ Correct method: Loop until EOF
            point_count = 0
            ...
            while True:
                ...
                try:
                    # ✅ Added endian specification: '<' = little-endian
                    ...
                    if len(xyz_bytes) < 24:
                        ...
                        break
                    xyz = struct.unpack('<3d', xyz_bytes)  # ✅ Fixed to '<3d'
                    ...
                    if len(rgb_bytes) < 3:
                        ...
                        break
                    rgb = struct.unpack('<3B', rgb_bytes)  # ✅ Fixed to '<3B'
                    ...
                    if len(error_bytes) < 8:
                        ...
                        break
                    error = struct.unpack('<d', error_bytes)[0]  # ✅ Fixed to '<d'
                    ...
                    if len(track_len_bytes) < 8:
                        ...
                        break
                    track_length = struct.unpack('<Q', track_len_bytes)[0]  # ✅ Fixed to '<Q'
                    
                    # Track data (track_length * 8 bytes)
                    # Each entry: image_id (uint32=4bytes) + point2D_idx (uint32=4bytes) = 8 bytes
                    if track_length > 0:
    ...
    # ✅ Keep as float64 to maintain precision
    points = np.array(points, dtype=np.float64)
    colors = np.array(colors, dtype=np.uint8)
    
    # ✅ Validate for Inf/NaN values and remove them
