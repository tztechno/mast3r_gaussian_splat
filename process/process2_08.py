...
def extract_colors_from_images_optimized(image_paths, pts3d, conf_raw, conf_threshold=1.5):
    ...
    for idx, img_path in enumerate(image_paths):
        ...
        # 【重要】モデルの出力（conf_raw / 30枚）とサイズを合わせる
        # 1画像あたりのポイント数を算出 (1474560 / 30 = 49152)
        pts_per_image = len(conf_raw) // len(image_paths)
        # 画像から抽出した色のうち、モデルの出力点数分だけを保持
        all_colors.append(colors_flat[:pts_per_image])

    # 全画像を結合
    colors_all = np.vstack(all_colors)
    print(f"✓ Total aligned colors: {len(colors_all):,}")

    # conf_raw とサイズが一致するはずなので、フィルタリングを適用
    valid_mask = conf_raw > conf_threshold
    colors_filtered = colors_all[valid_mask]

    print(f"✓ Colors after filtering: {len(colors_filtered):,}")

    # さらに Step 1 で行われた「ダウンサンプリング」に対応させる
    if len(colors_filtered) != len(pts3d):
        print(f"⚠️ Adjusting for downsampling: {len(colors_filtered)} -> {len(pts3d)}")
        # 注意: もし Step 1 でランダムサンプリング（np.random.choice）をしている場合、
        # 同じ random.seed を使わないと色がズレますが、
        # 数を合わせるためにスライスまたはリサンプリングを行います。
        if len(colors_filtered) > len(pts3d):
            # 簡易的に先頭から取得（本当はインデックスを一致させるのが理想）
            colors_filtered = colors_filtered[:len(pts3d)]
        else:
            # 足りない場合はpts3dの方を削る（滅多にないケース）
            pass 
